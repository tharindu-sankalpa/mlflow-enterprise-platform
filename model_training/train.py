...
def main():
    """
    Main function for the Adult Income Classification training job.
    
    This function handles the complete workflow including:
    1. Command-line argument parsing
    2. MLflow configuration
    3. Data loading and preprocessing
    4. Model training for multiple classifiers
    5. Model evaluation and logging
    6. Artifact saving
    
    The function supports training various machine learning models including
    Logistic Regression, Random Forest, XGBoost, Gradient Boosting, KNN,
    and optionally a TensorFlow Deep Neural Network.
    """
...
def train_tensorflow_model(X_train, X_test, y_train, y_test, output_dir):
    """
    Train a TensorFlow Deep Neural Network model.
    
    This function creates, trains, and evaluates a TensorFlow DNN model
    for binary classification. The model architecture consists of two hidden layers
    with dropout for regularization and a sigmoid output layer for binary classification.
    Training is monitored with early stopping to prevent overfitting.
    
    Parameters:
        X_train (numpy.ndarray): Training feature data (already preprocessed)
        X_test (numpy.ndarray): Testing feature data (already preprocessed)
        y_train (numpy.ndarray): Training target data
        y_test (numpy.ndarray): Testing target data
        output_dir (str): Directory where model will be saved
        
    Returns:
        None: Model and metrics are logged to MLflow automatically
    """
